...
namespace RoslynSample
{
    public class Driver
    {
	    static void Main()
	    {
			...
			var syntaxTree = CSharpSyntaxTree.ParseText(@"class C { void Foo() {
	int i = 10;  
	i = i + 1; /* My Comment */ 
} }");
	...
	internal class UpdaterVisitor : CSharpSyntaxRewriter
	{
		...
		{
			var stringToken = SyntaxFactory.Token(SyntaxKind.StringKeyword).WithTrailingTrivia(SyntaxFactory.Space);

			var newField = SyntaxFactory.FieldDeclaration(
							SyntaxFactory.VariableDeclaration(
								SyntaxFactory.PredefinedType(stringToken),
								SyntaxFactory.SeparatedList<VariableDeclaratorSyntax>().Add(SyntaxFactory.VariableDeclarator("foo"))));
